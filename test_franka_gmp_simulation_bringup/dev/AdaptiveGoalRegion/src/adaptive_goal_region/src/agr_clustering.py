...
def cluster_by_orientation(
    ...
    labels = db.labels_
    print(
        f"Estimated number of clusters of orientation: {len(set(labels)) - (1 if -1 in labels else 0)}"
    )
    cluster_indices = []
...
def grasping_poses_to_position_and_orientation(
    pred_grasps_cam: np.ndarray,
...
def generate_indices_array(
    labels: np.ndarray, n_clusters: int, positions: np.ndarray, orientations: np.ndarray
...
def generate_indices_array_new(
    labels: np.ndarray, positions: np.ndarray, orientations: np.ndarray
) -> Tuple[np.ndarray, Dict]:
    global_labels = {}
    indices = []
    last_index = 0
